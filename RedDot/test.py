...
class MoveToCar:
    ...
    def red_dectection(self, frame):
        ...
        if areas:
            ...
            # target is found
            if areas[max_index] > self.min_target_area:
		...
		if self.vehicle_state == 0:
		    # check three times before locking up this target
		        ...
		        self.target_found = True
                        ...
                        filename = "Test/Searching: " + datetime.datetime.now().strftime("%m-%d %H:%M:%S--") + "x:"+ str(self.xpos) +" y:"+ str(self.ypos) + " A:" + str(Area) + ".jpg"
		    ...
		    filename = "Test/Moving: " + datetime.datetime.now().strftime("%m-%d %H:%M:%S--") + "x:"+ str(self.xpos) +" y:"+ str(self.ypos) + " A:" + str(Area) + ".jpg"
		    cv2.imwrite(filename, frame)
            else:
